...
def ejemplo1():
    ...
    print(informacion)
    miArchivo.close() # Recomendado siempre cerrar el archivo

#ejemplo1()

def ejemplo1_2():
    with open("miarchivo.txt", 'r') as miArchivo:
        print(miArchivo.read()) # No es necesario colocar close
...
def ejemplo2():
    ...
    print(linea)
    print("Leí la primera línea. Vamos a la segunda:")
    linea = miArchivo.readline()
    print(linea)
    miArchivo.seek(0)
    print("Vuelvo a la primera línea:")
    linea = miArchivo.readline()
    print(linea)
    print("Mostrar la lína 2:")
    lineas2 = miArchivo.readlines(2)
    print(lineas2)
    miArchivo.close()
    print("Trae todas las líneas como una lista:")
    miArchivo = open('miarchivo.txt', 'r')
...
def actividad1():
    ...
    matrizfinal = []
    for item in miArchivo:
        matrizfinal.append(item.rstrip('\n').split(","))
    print(matrizfinal)
#
