...
def actividad2(suma):
    ...
    with open("recibo.txt", 'a') as recibo:
        guardaProducto(nombre_producto, precio_producto, recibo)
    suma += int(precio_producto)
    entrada = input("¿Desea ingresar más productos? (s/n): ")
    if entrada == "s" or entrada == "S":
        actividad2(suma)
    else:
        print("Gracias por su compra.")
        print("El total de la compra fue: $" + str(suma))
